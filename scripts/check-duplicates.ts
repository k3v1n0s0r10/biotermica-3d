...
if (report.kind !== 'dupes' || !Number.isInteger(report.stats?.clone_groups)) {
  throw new Error('Unexpected Fallow duplication report; review the installed tool schema.');
}
if (report.stats.clone_groups > 0) {
  console.error(JSON.stringify(report, null, 2));
  console.error('Duplicate code is blocking. Inspect with bunx --no-install fallow dupes.');
  process.exit(1);
}

...
test('air is fine and dense; water uses distinct liquid meshes with repeatable motion', () => {
  ...
  for (const [time, stage] of [
    ...
    if (system instanceof LineSegments) {
      expect(system.geometry.getAttribute('position').count).toBeGreaterThan(
        1000,
      );
  ...
  for (const [time, stage] of [
    ...
    const stream = requireValue(flow.root.children[stage]).getObjectByName(
      'water-surface',
    ) as Mesh;
    flow.sample(time);
    ...
    expect(Array.from(positions.array).slice(0, 75)).toEqual(
      expected.slice(0, 75),
    );
    flow.sample(2);
    flow.sample(23);

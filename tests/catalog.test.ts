...
import { animations, models, showcases, resolveSelection } from '../src/library/catalog';
...
test('all catalog references resolve and IDs are unique within each collection', () => {
  ...
  for (const film of showcases) expect(models.some(model => model.id === film.modelId)).toBe(true);
...
test('stale selection links fall back to valid items while valid animation selections survive', () => {
  const fallback = resolveSelection({ mode: 'studio', modelId: 'removed-model', animationId: 'removed-animation', showcaseId: 'removed-film' });
  expect(models.some(model => model.id === fallback.modelId)).toBe(true);
  expect(animations.find(animation => animation.id === fallback.animationId)?.modelIds).toContain(fallback.modelId);
  ...
  expect(film.modelId).toBe(showcases.find(entry => entry.id === film.showcaseId)!.modelId);
});

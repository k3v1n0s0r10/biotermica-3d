import {
  BoxGeometry, CylinderGeometry, DoubleSide, ExtrudeGeometry, Group, InstancedMesh,
  Mesh, MeshStandardMaterial, Object3D, Path, Shape, TorusGeometry,
  type BufferGeometry,
} from 'three';
...
export function createHeatPump() {
  ...
  function box(name: string, w: number, h: number, d: number, x: number, y: number, z: number, material = paint, parent = product) {
  ...
  const base = mesh('base-pan', new ExtrudeGeometry(outline(lidHalf, lidRadius), {
    depth: 0.045, bevelEnabled: false, curveSegments: 16,
  }), edge);
  ...
  function ventPanel(width: number, name: string) {
    ...
    shape.lineTo(width / 2, -0.0175); shape.lineTo(width / 2, height);
    ...
    mesh('perforated-sheet', new ExtrudeGeometry(shape, { depth: 0.007, bevelEnabled: false, curveSegments: 4 }), paint, panel);
    ...
    for (let i = 0; i < 150; i++) {
      ...
      transform.updateMatrix(); fins.setMatrixAt(i, transform.matrix);
    }
    panel.add(fins);
    return panel;
  }
  ...
  front.position.set(-0.092, 0.105, 0.447); product.add(front);
  const right = ventPanel(0.634, 'right-grille');
  right.rotation.y = Math.PI / 2; right.position.set(0.447, 0.105, -0.092); product.add(right);
  const back = ventPanel(0.818, 'back-grille');
  back.rotation.y = Math.PI; back.position.set(0, 0.105, -0.447); product.add(back);
  const left = ventPanel(0.818, 'left-grille');
  left.rotation.y = -Math.PI / 2; left.position.set(-0.447, 0.105, 0); product.add(left);
  ...
  const cornerGeometry = new ExtrudeGeometry(rolledCorner, {
    depth: 1.065 - 0.0875, bevelEnabled: false, curveSegments: 24,
  });
  ...
  const lid = new Group(); lid.name = 'electrical-lid'; product.add(lid);
  ...
  const lidCorner = mesh('lid-rounded-corner', new ExtrudeGeometry(cornerSection, {
    depth: 0.812, bevelEnabled: false, curveSegments: 16,
  }), paint, lid);
  ...
  for (const y of [0.285, 1.015]) for (const x of [0.248, 0.389]) {
    const screw = mesh('lid-fastener', new CylinderGeometry(0.004, 0.004, 0.003, 12), steel, lid);
    screw.rotation.x = Math.PI / 2; screw.position.set(x, y, 0.456);
  ...
  const aperture = new Path(); aperture.absarc(0, 0, 0.381, 0, Math.PI * 2, true); deck.holes.push(aperture);
  const top = mesh('top-deck', new ExtrudeGeometry(deck, { depth: 0.019, bevelEnabled: false, curveSegments: 64 }), edge);
  top.rotation.x = -Math.PI / 2; top.position.y = 1.065;
  const shroudMaterial = black.clone();
  shroudMaterial.side = DoubleSide;
  const shroud = mesh('fan-shroud', new CylinderGeometry(0.38, 0.36, 0.08, 96, 1, true), shroudMaterial);
  shroud.position.y = 1.03;
  const fan = new Group(); fan.name = 'fan-rotor'; fan.position.y = 1.01; product.add(fan);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    bladeShape.quadraticCurveTo(0.13, 0.06, 0.045, 0.025); bladeShape.closePath();
    const bladeGeometry = new ExtrudeGeometry(bladeShape, { depth: 0.004, bevelEnabled: false, curveSegments: 16 });
  ...
  const guard = new Group(); guard.name = 'fan-guard'; product.add(guard);
  for (let i = 0; i < 12; i++) {
    const ring = mesh('guard-ring', new TorusGeometry(0.095 + i * 0.0253, 0.0024, 6, 128), edge, guard);
    ring.rotation.x = Math.PI / 2; ring.position.y = 1.096;
  }
  for (let i = 0; i < 12; i++) {
  ...
  for (const x of [-0.421, 0.421]) for (const z of [-0.421, 0.421]) {
  ...
  product.add(stand);
  for (const x of [-0.37, 0.37]) for (const z of [-0.37, 0.37]) {
